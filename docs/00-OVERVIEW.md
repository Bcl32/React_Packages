...
| [`@bcl32/themes`](./01-packages/) | mid | `5.0.0` | HSL-based theming system: theme provider/persistence, a shared Tailwind preset (`./tailwind-preset`) that generates the palette for consumer apps and binds the `dark:` variant to `data-theme` rather than the OS colour scheme, live CSS-variable editor UI, `isLightTheme`/`LIGHT_THEMES` derivation, and hex/RGB/HSL colour utilities. |
