...
| `useDataTableFilterBar` | hook | `({ filters, changeFilters, activeFilters, filteredCount, totalCount, addFilter?, removeFilter?, filterCatalog?, searchIndex?, columns?, columnVisibility?, hasPrimaryFilters?, hiddenFields?, collapsible?, size? }) => DataTableFilter` | Builds the filter bar: a search box, the "+ Add filter" picker, a "Filters" toggle and one chip per active filter (`toolbar`), and the card panel (`panel`). `collapsible` (default `true`) puts the panel behind the toggle; `false` drops the toggle, keeps the cards always on screen, renders nothing when no card is mounted, and makes keyboard targeting's `reveal` scroll the bar into view instead of unfolding it. `size` (`"default"` \| `"large"`) scales the search box, picker, toggle and chips; `large` chips gain a fill, ring and weight — pair it with `PageFilterBar`. The result also carries each piece on its own — `search`, `addPicker`, `toggle`, `chips`, `activeCount` — for a page that arranges them itself. **Creates its own `FilterProvider` internally.** |
| `PageFilterBar` | component | `({ filter: DataTableFilter, title?, leading?, trailing?, className? }) => JSX.Element` | Lays a `collapsible: false` bar out at the top of a page: one wrapping row of `title` "(shown/total)" · `leading` · search · add-picker · chips · `trailing` (pushed right), the cards beneath. The table beneath then takes no `title` (optional in `@bcl32/datatable` ≥ 2.19) and no `filter`. For pages whose filters govern more than the table — a colour wheel, a map, facet rows. |
...
| `DataTableFilter` | type | `{ toolbar, panel, filteredCount, totalCount, search, addPicker, toggle, chips, activeCount }` | Return shape of `useDataTableFilterBar`. `toolbar`/`panel` are the composed nodes `DataTable` renders; `search`/`addPicker`/`toggle`/`chips` are the same pieces individually (each `null` when absent). Render a piece *or* the composed `toolbar`, never both. |
