...
| `apiFetch` | util | `(input: RequestInfo \| URL, opts?: ApiFetchOptions) => Promise<Response>` | `fetch` wrapper that throws `ApiError` on any non-OK response — and on failures with no HTTP response at all: offline/DNS/TLS rejections become `ApiError{status: 0, code: 'network_error'}`, and an elapsed `opts.timeoutMs` becomes `ApiError{status: 0, code: 'request_timeout'}` (caller aborts are re-thrown untouched). Handles the new `{code, message, details}` envelope, legacy FastAPI `{detail: string}`, structured `{detail: {code, message}}`, and FastAPI 422 validation arrays (formatted as `'field: reason'`). |
| `ApiFetchOptions` | type | `interface ApiFetchOptions extends RequestInit { timeoutMs?: number \| null }` | `RequestInit` plus `timeoutMs`. No timeout by default — long-running mutations must not be cut off silently; `useGetRequest` applies a 30s default for GETs. Composes with a caller `signal` via `AbortSignal.any` where available. |
| `useGetRequest` | hook | `<T = unknown>(url: string, options?: { queryKey?: string[]; enabled?: boolean; staleTime?: number; responseType?: 'json' \| 'text'; timeoutMs?: number \| null }) => UseQueryResult<T, ApiError>` | TanStack Query wrapper for GET requests. `queryKey` defaults to `[url]`. Supports custom `queryKey`, `enabled` flag, `staleTime`, `responseType`, and `timeoutMs` (default 30s; pass `null` to disable, or raise it for known-slow reads like large meshes). |
| `defaultShouldRetry` | util | `(failureCount: number, error: unknown) => boolean` | Shared retry predicate: never retries 4xx or `request_timeout`; retries 5xx and `network_error` (status 0) up to 2×; one cautious retry for non-`ApiError` shapes. |
| `defaultRetryDelay` | util | `(attempt: number) => number` | Exponential backoff capped at 5s (1s, 2s, 4s, 5s…). |
| `sharedQueryRetry` | const | `{ retry: defaultShouldRetry, retryDelay: defaultRetryDelay }` | Spread into a QueryClient's `defaultOptions.queries`. Without it apps inherit TanStack's default `retry: 3`, which re-asks deterministic 404s for ~7s. If the app later calls `setDefaultOptions`, spread `getDefaultOptions().queries` first — it replaces the whole object. |
| `useApiMutation` | hook | `<TData = unknown, TResponse = unknown>(url: string, options?: { method?: 'POST'\|'PUT'\|'PATCH'\|'DELETE'; invalidateKeys?: string[]; onError?: (error, variables, context) => void; onSuccess?: (data, variables, context) => void; meta?: Record<string, unknown> }) => UseMutationResult<TResponse, ApiError, TData>` | Mutation hook where the payload is passed at `.mutate(data)` call time. Defaults to `POST`. Auto-detects `File`/`Blob` in the payload and switches to `multipart/form-data`. Optionally invalidates `invalidateKeys` on success (caller `onSuccess` runs after the invalidation). Hook-level `onError` is also the opt-out signal for an app-wide `MutationCache.onError` default toast — global handlers can't see mutate-time callbacks, so error handling belongs in the hook options; `meta: { silenceErrorToast: true }` opts out without a handler. |
